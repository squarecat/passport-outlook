...
function Strategy(options, verify) {
  ...
  this._userProfileURL =
    options.userProfileURL || 'https://outlook.office.com/api/v2.0/me';
...
Strategy.prototype.authorizationParams = function(options) {
  ...
  ['locale', 'display', 'prompt', 'login_hint'].forEach(function(name) {
    if (options[name]) {
...
Strategy.prototype.userProfile = function(accessToken, done) {
  this._oauth2.get(this._userProfileURL, accessToken, function(err, res, body) {
    ...
    if (res && res.statusCode === 404) {
      return done(
        new OutlookAPIError(res.headers['x-caserrorcode'], res.statusCode)
      );
    }
